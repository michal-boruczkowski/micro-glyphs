...
> {
    ...
    enter(fn: (selection: Selection<GElement, Datum, any, PDatum>) => any): this;
    update(fn: (selection: Selection<GElement, Datum, any, PDatum>) => any): this;
    exit(fn: (selection: Selection<GElement, Datum, any, PDatum>) => any): this;
    merged(fn: (selection: Selection<GElement, Datum, any, PDatum>) => any): this;
}
...
>(tag: string, className?: string): D3Component<GElement, Datum, PDatum> {
    ...
    const render = function <PGE extends BaseType, PPGE extends BaseType, PPD>(
        ...
        const merged = joined.join(
            ...
            (update: Selection<GElement, Datum, any, PDatum>) => {
                update.call(_updateFn)
                return update;
            },
            (exit: Selection<GElement, Datum, any, PDatum>) => {
                exit.call(_exitFn)
                return exit;
            }
        );
